...
# import network # REMOVED: No longer needed
...
config = {
    ...
    "connect_coro": eliza,
    # "wifi_coro": eliza, # REMOVED
    # "ssid": None, # REMOVED
    # "wifi_pw": None, # REMOVED
    "queue_len": 0,
    "gateway": False,
    "mqttv5": False,
...
class MQTT_base:
    ...
    def __init__(self, config):
        ...
        if self._events:
            ...
            # self._wifi_handler = config["wifi_coro"] # REMOVED
        ...
        self._sock = None
        # self._sta_if = network.WLAN(network.STA_IF) # REMOVED
        # self._sta_if.active(True) # REMOVED
        if config["gateway"]:  # Called from gateway (hence ESP32).
            import aioespnow  # Set up ESPNOW

            # REMOVED Wi-Fi specific configuration for gateway
        ...
        self._ssl = config["ssl"] # MOVED up from MQTTClient
        self._ssl_params = config["ssl_params"] # MOVED up from MQTTClient
        # DNS lookup moved here
    ...
    async def _as_read(self, n, sock=None):  # OSError caught by superclass
        ...
        t = ticks_ms()
        while size < n:
            if self._timeout(t) or not self.isconnected(): # Changed condition
    ...
    # REMOVED: No longer checking WAN connectivity
    # async def wan_ok(
    #     self,
    ...
    # Can raise OSError if WiFi fails. Subclass traps. # Changed: No longer referring to Wi-Fi
    ...
    async def wait_msg(self):
        mqttv5 = self.mqttv5  # Cache local
        try:
            res = self._sock.read(1)  # Throws OSError on WiFi fail # Changed: No longer referring to Wi-Fi
        except OSError as e:
...
class MQTTClient(MQTT_base):
    ...
    async def connect(self, *, quick=False):  # Quick initial connect option for battery apps
        # REMOVED: Initial wifi_connect. The user is now responsible for network connectivity.
        ...
        if not self._events:
            # asyncio.create_task(self._wifi_handler(True))  # User handler. # REMOVED
            pass # No wifi handler to call
        if not self._has_connected:
    ...
    async def _handle_msg(self):
        try:
            ...
            pass
        self._reconnect()  # Broker fail. # Changed: Removed "or WiFi fail."
    ...
    def isconnected(self):
        ...
        # Simplified to only check MQTT connection status
        # if self._isconnected and not self._sta_if.isconnected():  # It's going down. # REMOVED
        #    self._reconnect() # REMOVED
    ...
    def _reconnect(self):  # Schedule a reconnection if not underway.
        if self._isconnected:
            ...
            if self._events:  # Signal an outage
                ...
                # asyncio.create_task(self._wifi_handler(False))  # User handler. # REMOVED
                pass # No wifi handler to call
    ...
    # Scheduled on 1st successful connection. Runs forever maintaining
    # broker connection.
    async def _keep_connected(self):
        while self._has_connected:
            if self.isconnected():  # Pause for 1 second
                ...
                # REMOVED: Wi-Fi disconnect/reconnect logic
                # try:
                #     self._sta_if.disconnect()
                # except OSError:
                #     self.dprint("Wi-Fi not started, unable to disconnect interface")
                await asyncio.sleep(1)
                # REMOVED: No wifi_connect here
                # try:
